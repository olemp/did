 ...
 */
export function useStartTeamsConversation({
  users,
  period,
  topic
}: ITeamsReminderButtonProps) {
  ...
  const startTeamsConversation = () => {
    ...
    if (teamsSettings?.missingSubmissionsSinglePeriodText) {
      message = format(
        teamsSettings.missingSubmissionsSinglePeriodText,
        period?.name
      )
    }
    if (first(users).periods) {
      ...
      if (teamsSettings?.missingSubmissionsMultiplePeriodsText) {
        message = format(
          teamsSettings.missingSubmissionsMultiplePeriodsText,
          periods
        )
      }
    }
