...
export function createTimesheetReducer({ url, t }: ITimesheetReducerParameters) {
  return createReducer<ITimesheetState>(initState(url), (builder) => builder
    .addCase(DATA_UPDATED, (state, { payload }) => {
      ...
      if (payload.query.data) {
        ...
        state.periods = payload.query.data.periods.map(
          (period: TimesheetPeriodObject) => new TimesheetPeriod().initialize(period)
        )
    ...
    .addCase(MANUAL_MATCH, (state, { payload }) => {
      ...
      state.periods = state.periods.map((p) => p.id === state.selectedPeriod.id ? state.selectedPeriod : p
      )
    })
